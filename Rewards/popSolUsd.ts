 ...
 */
export const populateSolUsd = async (): Promise<void> => {
  try {
    // Fetch all rewards from the database
    const rewards: IReward[] = await Reward.find();
    ...
    for (let i = 0; i < rewards.length; i++) {
      ...
      const solUsd: number = await fetchSolanaPriceAtDate(rewards[i].timestamp);
      ...
      logger.info(
        `Successfully updated solUsd for reward ID: ${rewards[i]._id.toString()}`,
      );
    }
